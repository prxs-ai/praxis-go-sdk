...
func main() {
	...
	case "update-agent":
		...
		addr := common.Address{}
		if *newAddress != "" {
			addr = common.HexToAddress(*newAddress)
		}
	...
	case "sign-registration":
		...
		domain := fs.String("domain", "", "agent domain")
		fs.Parse(os.Args[2:])
		if *pkhex == "" || *agentID == 0 || *addr == "" || *domain == "" {
			log.Fatal("missing args")
		}
		priv, err := crypto.HexToECDSA(strings.TrimPrefix(*pkhex, "0x"))
		if err != nil {
	...
	case "resolve-domain":
		...
		ident, err := erc.NewIdentity(common.HexToAddress(*idAddr), ec)
		if err != nil {
			log.Fatal(err)
		}
		...
		if err != nil {
			...
			ai.AgentId = li.AgentId
			ai.AgentDomain = li.AgentDomain
			ai.AgentAddress = li.AgentAddress
		}
	...
	case "resolve-address":
		...
		ai, err := ident.ResolveByAddress(context.Background(), &bind.CallOpts{}, common.HexToAddress(*addr))
		if err != nil {
			log.Fatal(err)
		}
		out := map[string]any{"agentId": ai.AgentId.String(), "domain": ai.AgentDomain, "address": ai.AgentAddress.Hex()}
...
func findByDomainLogs(ctx context.Context, ec *ethclient.Client, idAddr common.Address, domain string) (erc.AgentInfo, error) {
	...
	if last.AgentId == nil {
		return erc.AgentInfo{}, fmt.Errorf("not found in logs")
	}
	return last, nil
}
