...
func (o *OrchestratorAnalyzer) convertLLMPlanToAST(plan *llm.WorkflowPlan) (*AST, map[string]interface{}) {
	...
	for _, node := range plan.Nodes {
		o.logger.Infof("🔍 LLM Plan Node: Type=%s, ToolName=%s, Args=%v",
			node.Type, node.ToolName, node.Args)
		...
		if node.Type == "tool" && node.ToolName != "" {
			...
			for k, v := range node.Args {
				...
				// Detect secrets vs params
				if strings.HasPrefix(k, "secret.") {
					key := strings.TrimPrefix(k, "secret.")
					secrets[key] = v
					o.logger.Infof("🔑 Detected secret: %s", key)
				} else {
					params[k] = v
					o.logger.Infof("📦 Detected param: %s = %v", k, v)
				}
			}

			// Add sensible defaults for common tools
			switch node.ToolName {
			case "list_files":
				if _, exists := params["directory"]; !exists {
					params["directory"] = "/shared"
			...
			case "write_file":
				...
				if _, exists := params["content"]; !exists {
					params["content"] = "Content generated by AI orchestrator"
			...
			astNode := ASTNode{
				Type:     NodeTypeCall,
				Value:    "CALL", // For backward compatibility
				ToolName: node.ToolName,
				Args:     argsMap, // legacy placeholder
				Params:   params,
				Secrets:  secrets,
			}

			o.logger.Infof("✅ Generated AST node for %s with %d params, %d secrets",
				node.ToolName, len(params), len(secrets))

			ast.Nodes = append(ast.Nodes, astNode)
		} else {
			o.logger.Warnf("⚠️ Skipping invalid LLM node: Type=%s, ToolName=%s",
				node.Type, node.ToolName)
		}
	}
