...
type P2PForgeCertMgr struct {
	...
	privKey                    crypto.PrivKey // explicitly stored private key (workaround for peerstore)
...
// WithPrivKeySource allows specifying explicit libp2p private key accessor in case peerstore lookup fails.
func WithPrivKeySource(fn func() crypto.PrivKey) P2PForgeCertMgrOptions {
	return func(config *P2PForgeCertMgrConfig) error {
		config.privKeyFn = fn
		return nil
	}
...
func NewP2PForgeCertMgr(opts ...P2PForgeCertMgrOptions) (*P2PForgeCertMgr, error) {
	...
	// Create privKeyFn: use explicit if provided, else fallback to peerstore
	privKeyFn := mgrCfg.privKeyFn
	if privKeyFn == nil {
		privKeyFn = func() crypto.PrivKey {
			if mgr.privKey != nil {
				return mgr.privKey
...
// ProvideHostAndPrivKey provides both host and private key explicitly.
// This is a workaround for peerstore.PrivKey() returning nil in some cases.
// Use this method instead of ProvideHost when you have the private key available.
func (m *P2PForgeCertMgr) ProvideHostAndPrivKey(h host.Host, privKey crypto.PrivKey) {
	m.privKey = privKey
	m.ProvideHost(h)
}
...
type dns01P2PForgeSolver struct {
	...
	forgeAuth                  string
	hostFn                     func() host.Host
	privKeyFn                  func() crypto.PrivKey // function to get private key (workaround for peerstore)
	modifyForgeRequest         func(r *http.Request) error
	userAgent                  string
	allowPrivateForgeAddresses bool
	log                        *zap.SugaredLogger
...
func (d *dns01P2PForgeSolver) Present(ctx context.Context, challenge acme.Challenge) error {
	...
	// Try explicit privKey first (workaround for peerstore issues)
	privKey := d.privKeyFn()
	if privKey == nil {
		return fmt.Errorf("private key not available: not in peerstore and not explicitly provided via ProvideHostAndPrivKey()")
	}
